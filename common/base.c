...
REALIGN_STACK void x264_param_default( x264_param_t *param )
{
    ...
    param->SSIM1 = 0;
    param->GAUSSIAN1 = 0;
    param->GAUSSIAN1_STRENGTH = 0.3;//默认为0.3
    param->JND1 = 0;
	...
	param->WEIGHTED_GUIDED_FILTER1 = 0;
    param->WEIGHTED_GUIDED_FILTER1_STRENGTH = 0.2;
	param->GAMMA1 = 0;
...
static int param_apply_preset( x264_param_t *param, const char *preset )
{
    ...
    if( !strcasecmp( preset, "ultrafast" ) )
    {
        ...
        param->rc.i_aq_mode = 0;//如果想在ultrafast模式下让其运行，把这个值改为非0值 encoder.c1177行应该改为clip（0,4）因为我们自己新增了一个模式4
...
#define OPT(STR) else if( !strcmp( name, STR ) )//若str1=str2，则返回零；若str1str2，则返回正数
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    OPT("gauss-filter")
    {
        ...
        printf("gauss-filter start  gaussian strength = %f\n", p->GAUSSIAN1_STRENGTH);
    }
	OPT("his-equ")
    {
	...
	OPT("guided")
    {
        p->WEIGHTED_GUIDED_FILTER1 = 1;
        p->WEIGHTED_GUIDED_FILTER1_STRENGTH = atof(value);
        printf("guided start,WEIGHTED_GUIDED_FILTER1_STRENGTH=%f\n",p->WEIGHTED_GUIDED_FILTER1_STRENGTH);
    }
	OPT("gamma-sa")
    {
        p->GAMMA1 = atoi(value);
