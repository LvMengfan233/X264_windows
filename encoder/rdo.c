...
static inline int ssd_mb(x264_t* h)
{
	...
	//i_ssd = (int)(1.75 * (double)(i_ssd));//调整亮度Distortion
	if (CHROMA_FORMAT)
	{
		int chroma_size = h->luma2chroma_pixel[PIXEL_16x16];
		int chroma_ssd = ssd_plane(h, chroma_size, 1, 0, 0) + ssd_plane(h, chroma_size, 2, 0, 0);   //源代码
		//double y = (double)(ssd_plane(h, chroma_size, 1, 0, 0));
		//printf("y=%f\n", y);
		//double u = (double)(ssd_plane(h, chroma_size, 2, 0, 0));
		//printf("u=%f\n", u);
		//int chroma_ssd = (int)(0.65 * (double)(ssd_plane(h, chroma_size, 1, 0, 0))) + (int)(0.88 * (double)(ssd_plane(h, chroma_size, 2, 0, 0)));  //分别调整U、V  Distortion
		//printf("chroma_ssd=%d\n", chroma_ssd);
		//chroma_ssd = (int)(0.3 * (double)(chroma_ssd));//调整色度Distortion
