...
#define GAUSSIAN_ADJ    0
#define GAUSSIAN_SIZE   8

#define GAUSSIAN_VARIANCE    0
#define GAUSSIAN_LAPLACE     0
	...
	if (h->param.JND1 == 1) {
		...
		for (int j = 0; j < Lum_height - 4; j++)
		{
			for (int i = 0; i < Lum_width - 4; i++)
			{
				...
				//}
				//else
				Lum_sjndCur[i + (j + 2) * Lum_width + 2] = Lum_f2Cur[i + (j + 2) * Lum_width + 2];
			}
		}
